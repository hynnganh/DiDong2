...
# # 1. API Key và Cấu hình Gemini
# API_KEY = "AIzaSyCrRy_rsuOcXkt737v8qP1HATEwEMugTKA" 
...
#         user="root",
#         password="",
#         database="ngocanhdidong",
#         autocommit=True # Tự động save dữ liệu
#     )

# # Lấy kho hàng để AI tư vấn (Lấy thêm ID và Image để hiển thị)
...
#     except Exception as e:
#         print(f"Lỗi lấy kho: {e}")
#         return "", []
#     finally:
#         if db: db.close()

# # Hàm xử lý thêm vào giỏ hàng SQL
...
#         else:
#             # Nếu chưa có thì thêm mới (giả sử discount mặc định là 0)
#             sql = "INSERT INTO cart_items (cart_id, product_id, product_price, quantity, discount) VALUES (%s, %s, %s, %s, %s)"
#             cursor.execute(sql, (cart_id, product_id, price, 1, 0.0))

#         # 2. Cập nhật lại tổng tiền trong bảng carts (nếu nàng có cột total_price)
#         cursor.execute("UPDATE carts SET total_price = COALESCE(total_price, 0) + %s WHERE cart_id = %s", (price, cart_id))
        
#         return True
#     except Exception as e:
#         print(f"Lỗi SQL Add Cart: {e}")
#         traceback.print_exc()
#         return False
#     finally:
#         if db: db.close()
...
#         cart_id = data.get("cartId") # FE gửi kèm cartId xuống nhé

#         if not cart_id:
#             return jsonify({"status": "error", "reply": "Nàng ơi, đăng nhập để mua hàng nhé! ✨"}), 400

#         # Bước 1: Lấy bối cảnh sản phẩm
#         context_data, raw_products = get_product_context()

#         # Bước 2: Tạo Prompt với quy tắc nghiêm ngặt
#         prompt = f"""
#         Bạn là Ngọc Anh Beauty AI. Hãy xưng hô 'em' và gọi khách là 'nàng'.
#         Dữ liệu sản phẩm thực tế:
#         {context_data}

#         QUY TẮC PHẢN HỒI:
#         1. Nếu nàng muốn mua hoặc thêm vào giỏ: Trả về câu thông báo và bắt buộc kèm dòng: ACTION: ADD_TO_CART | ID: [ID] | PRICE: [GIÁ]
#         2. Nếu nàng hỏi tư vấn: Hãy trả lời ngọt ngào dựa trên dữ liệu trên.
        
#         Câu hỏi của nàng: {user_msg}
#         """
...
#         # Bước 3: Kiểm tra xem AI có ra lệnh thêm giỏ hàng không
#         if "ACTION: ADD_TO_CART" in reply_text:
#             # Dùng Regex để tách ID và Price
...
#                 success = add_to_cart_sql(cart_id, p_id, p_price)
                
#                 if success:
#                     # Lấy thêm thông tin để FE hiển thị card đẹp
#                     p_data = next((p for p in raw_products if p['product_id'] == p_id), None)
#                     if p_data:
#                         product_info = {
#                             "id": p_id,
...
#                             "image": p_data['image']
#                         }
#                     # Xóa dòng lệnh ACTION đi để khách không thấy code
#                     reply_text = re.sub(r"ACTION:.*", "", reply_text).strip()
#                     reply_text += "\n\n✨ Em đã thêm vào giỏ hàng thành công cho nàng rồi nhé! ❤️"

#         return jsonify({
#             "status": "success",
#             "reply": reply_text,
#             "product_info": product_info # Gửi kèm để FE hiện Card
#         })
...
# Cấu hình Gemini
API_KEY = "AIzaSyDbackjHlZ087qpP-5KIaFJeG9j249VDA8" 
genai.configure(api_key=API_KEY)
def find_working_model():
    try:
        available_models = [m.name for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
